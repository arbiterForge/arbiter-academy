"""Build the fail-closed static public surface for Academy Preview 0.9."""
...
def build_preview_site(root: Path, out: Path, *, release_sha: str | None = None) -> None:
    """Render only the reviewed Preview 0.9 pages into *out*.
...
def _render_pages(
    ...
    pages: dict[Path, str] = {
        ...
        Path("recovery/index.html"): _page(
            templates,
            "Recovery | Arbiter Academy Preview 0.9",
    ...
    for position, (lab_id, lesson) in enumerate(lessons.items()):
        ...
        if next_lab in available:
            ...
            next_step = f"{escape(_lab_code(next_lab))} is not available in Academy Preview 0.9."
        previous_link = ""
        if position:
        ...
        pages[Path("labs") / lab_id / "index.html"] = _page(
            templates,
            f"{lesson['title']} | Arbiter Academy Preview 0.9",
