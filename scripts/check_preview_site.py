...
def _is_approved_external_url(target: str) -> bool:
    ...
    return True
_ASSET_SHA256 = {
    Path("assets/academy.css"): "a4c7848bdd3d76ac9981ccb336ac78630e95b7334c8b83dddcb2eaeacda6b2ea",
...
_EXPECTED_ACTION_IDS = {
    ...
    Path("labs/P08-repository-hygiene/index.html"): (
        "P08-return-to-main", "P08-prepare", "P08-inventory-native", "P08-inventory-harness-shell", "P08-run-standup", "P08-inventory-after-standup",
...
def _check_release(root: Path) -> str:
    ...
    if (
        ...
        or data.get("release") != "preview-0.30"
        ...
        raise ValueError("release.json does not contain the exact Preview 0.30 provenance contract")
    return data["release"]
...
def _check_publication_truth(root: Path, pages: dict[Path, _LinkCollector]) -> None:
    ...
    if runnable_links != expected_runnable_links:
        raise ValueError("home runnable lab links do not match the exact guided Preview 0.30 inventory")
    if tuple(home_collector.coming_next_entries) != _COMING_NEXT_ENTRIES:
        raise ValueError("home coming-next entries do not match the exact Preview 0.30 guided-rewrite sequence")
    ...
    for page, collector in pages.items():
        ...
        if tuple(collector.action_ids) != expected_actions:
            raise ValueError(
                f"generated action IDs do not match the exact Preview 0.30 contract: {relative.as_posix()}"
            )
        ...
        if tuple(collector.publication_statuses) != expected_statuses:
            raise ValueError(
                "generated publication status does not match the exact Preview 0.30 contract: "
                f"{relative.as_posix()}"
            )
