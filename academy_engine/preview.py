...
def validate_preview_manifest(
    ...
    """Validate an in-memory Preview 0.10 manifest against the raw Academy catalog."""
    if data is None:
    ...
    if runnable_labs != _RUNNABLE_LABS:
        raise ValueError("preview manifest runnable_labs contains lab(s) not eligible for Preview 0.10")
    _validate_guided_labs(catalog, runnable_labs, guided_labs)
    if guided_labs != _RUNNABLE_LABS:
        raise ValueError("preview manifest guided_labs must list only the reviewed F01 through F04 and P01 through P03 lessons")
    ...
    if prerequisites != _PREREQUISITES:
        raise ValueError("preview manifest prerequisites must match the reviewed Preview 0.10 onboarding contract")
    if known_limits != _KNOWN_LIMITS:
        raise ValueError("preview manifest known_limits must match the reviewed Preview 0.10 public limits")
...
def load_preview_manifest(root: Path) -> PreviewManifest:
    """Load and validate the checked-in Preview 0.10 public eligibility manifest."""
...
def require_runnable_lab(root: Path, lab_id: str) -> None:
    ...
    if lab_id not in manifest.runnable_labs:
        raise ValueError(f"{lab_id} is not runnable in Academy Preview 0.10")
...
def require_guided_lab(root: Path, lab_id: str) -> None:
    ...
    if lab_id not in manifest.guided_labs:
        raise ValueError(f"{lab_id} is not guided in Academy Preview 0.10")
