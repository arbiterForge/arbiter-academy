...
def _semantic(context: _SemanticContext) -> bool:
    ...
    if profile == "audit_guard_observation":
        ...
        try:
            ...
            return False
        event_line = lines[6].removeprefix("event_line: ") if len(lines) == 8 else ""
        return bool(
            len(lines) == 8
            and lines[:4] == [
            ...
            and lines[4] == "baseline_sha256: " + hashlib.sha256(prepared_overrides).hexdigest()
            and lines[5] == "event_sha256: " + hashlib.sha256(event_line.encode("utf-8")).hexdigest()
            and lines[6].startswith("event_line: ")
            and event_line == (
                "BLOCKED [H-05]: The .codearbiter audit logs (overrides.log, triage.log, "
                "sprint-log.md, gate-events.log, decisions/decision-log.md) are append-only "
                "(ORCHESTRATOR §7). Truncating, overwriting, or deleting the audit trail is "
                "prohibited; append with '>>' only."
            )
