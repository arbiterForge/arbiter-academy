...
## SD-ACA-020 - Promote U05 only with the released spike contract - confidence: high

- Date: 2026-08-13
- Point: Whether to leave the accepted U05 debug-and-spike lesson private, teach an Academy-specific approximation, or publish it with the released CodeArbiter findings-only spike workflow.
- Options: defer U05; retain an Academy-only spike receipt or branch; promote U05 in Preview 0.18 with its actual $ca-debug, $ca-spike, $ca-commit, and $ca-conflict boundaries.
- SMARTS: Safety and Security require a deleted `spike/u05-cache-key` branch, no merge or pull request, no copied spike code, and no fabricated command output. Reliability and Testability favor the real taskwrite board entry, committed findings-only transfer, exact published CLI lifecycle cards, and a source pin at CodeArbiter v2.15.1 rather than a predecessor with incompatible spike behavior. Maintainability and Simplicity preserve the shared lesson renderer and release machinery. Availability and Reviewability make the accepted U05 lane available without exposing U06 or U07.
- Chosen: publish U05 as the seventeenth guided lab in Preview 0.18 only after focused semantic, public-site, installer, asset, and hosted exact-head verification. Pin Academy's live CodeArbiter integration checks to v2.15.1 (`debb49da71aa1b97bca0988f72e46bb5875a23e3`); retain U06 and U07 as explicit non-routable coming-next lessons.

## SD-ACA-021 - Correct U02 to observe a real audit guard - confidence: high

- Date: 2026-08-14
- Point: Whether public U02 should continue requiring an invented training gate line, treat a permitted audit-log addition as a refusal, or demonstrate an H-05 content-neutral guard event while preserving protected audit bytes.
- Options: retain the fictional training flow; require a synthetic event; use the real H-05 observation and commit only a bounded note.
- SMARTS: Safety and Security preserve protected bytes and reject fabricated bypass evidence. Reliability and Testability require one refusal-shaped event, both digests, one note path, one commit, and a clean worktree while explicitly limiting provenance claims. Maintainability and Simplicity reuse the shared action-card renderer and current guard. Reviewability and Availability provide literal evidence fields, one selected-harness prompt, responsive controls, recovery, and a boundary between local evidence and human authorization.
- Chosen: correct U02 to use the real content-neutral H-05 observation. Publish it only in a fresh immutable successor preview after focused semantic, rendered, visual, release, hosted-CI, and Pages verification; current public U03-U07 routes remain unchanged.
