...
## Preview 0.3 quick start

Preview 0.3 publishes eleven reviewed Foundations and Practitioner labs, each
paced for 15–60 minutes. P08 and the Power User labs are not included. The
Preview is static, local training: it does not provide
...
3. Verify, snapshot, and install the verifier in sibling directories. The
   ...
   $academySource = Join-Path $academyParent "arbiter-academy-source-preview-0.3"
   $academyTools = Join-Path $academyParent "arbiter-academy-tools-preview-0.3"
   ...
   academy_source="$academy_parent/arbiter-academy-source-preview-0.3"
   academy_tools="$academy_parent/arbiter-academy-tools-preview-0.3"
...
Graduation is not available in Preview 0.3. The current receipt requires the
complete 19-lab course through U07, while this release intentionally publishes
only eleven verified labs. The `graduate` command remains reserved for the
