...
class PreviewSiteTests(unittest.TestCase):
    def test_preview_zero_twenty_seven_renders_f03_as_a_normal_guided_route(self) -> None:
        """Catches manifest promotion without the public F03 page and action payload."""
        manifest = load_preview_manifest(self.root)

        self.assertEqual(manifest.release, "preview-0.32")
        self.assertIn("F03-work-the-board", manifest.guided_labs)
        build_preview_site(self.root, self.out, release_sha="6" * 40)
        route = self.out / "labs" / "F03-work-the-board" / "index.html"
        self.assertTrue(route.is_file())
        rendered = route.read_text(encoding="utf-8")
        self.assertIn("F03-work-the-board", rendered)
        self.assertIn("Guided lesson", rendered)
        self.assertNotIn("Future private-source walkthrough", rendered)

    def test_preview_zero_twenty_seven_publishes_f03_in_the_guided_inventory(self) -> None:
        """Catches a promoted F03 path missing from the public Preview artifact."""
        ...
        manifest_path = publication / "preview-0.32.json"
        ...
        for lab_id, action_id in (
            ("P04-review-a-dependency", "P04-prepare"),
            ("P05-checkpoint-remediation", "P05-prepare"),
            ...
            ("U03-refactor-chore-release", "U03-check"),
            ("U04-initialize-projects", "U04-prepare-attempt"),
            ("U04-initialize-projects", "U04-write-binding-report"),
            ("U04-initialize-projects", "U04-check-status"),
            ("U05-debug-spike-conflict", "U05-prepare-attempt"),
            ("U05-debug-spike-conflict", "U05-run-debug"),
            ("U05-debug-spike-conflict", "U05-check-status"),
            ("U06-preview-and-advanced-surfaces", "U06-prepare-attempt"),
            ("U06-preview-and-advanced-surfaces", "U06-run-read-only-preview"),
            ("U06-preview-and-advanced-surfaces", "U06-check-status"),
            ("U07-capstone", "U07-prepare"),
            ("U07-capstone", "U07-run-feature"),
            ("U07-capstone", "U07-check"),
        ):
        ...
        self.assertIn("Preview 0.32", index)
        ...
        self.assertIn("Power User", u01)
        self.assertNotIn("Practitioner \u00b7 U01-autonomous-sprint", u01)
    ...
    def test_shared_release_chrome_derives_from_the_validated_preview_manifest(self) -> None:
        ...
        current = source / "academy" / "publication" / "preview-0.32.json"
        ...
        for page in (home, recovery, p08):
            ...
            self.assertNotIn("Preview 0.17", page)
        self.assertIn("<title>Arbiter Academy Preview 9.9</title>", home)
        self.assertIn("Preview 9.9 release</a>", home)
        self.assertIn('Continue with <a href="../U01-autonomous-sprint/index.html">U01-autonomous-sprint</a>.', p08)

    def test_f03_guided_document_renders_public_action_cards(self) -> None:
        """The public F03 document renders its runnable lifecycle and agent boundaries."""
        ...
        self.assertIn('data-action-id="F03-prepare"', html)
        ...
        self.assertIn('data-copy-target="command-F03-return-to-main-windows"', html)
        self.assertIn('git switch main &amp;&amp; git status --short --branch', html)
        self.assertIn('if ($LASTEXITCODE -ne 0) { throw &quot;git switch main failed&quot; }', html)
    ...
    def test_f03_staged_co_commit_review_renders_copyable_native_inspections(self) -> None:
        """Catches F03 asking for a staged review without a command a new learner can execute."""
        manifest = load_action_manifest(self.root, "F03-work-the-board")
        review = next(action for action in manifest.actions if action.id == "F03-review-co-commit-boundary")

        rendered = preview_site._render_action(review)
        ...
        for operating_system in ("windows", "macos", "linux"):
            with self.subTest(operating_system=operating_system):
                self.assertIn(
                    f'data-copy-target="command-F03-review-co-commit-boundary-{operating_system}"',
                    rendered,
                )
        self.assertIn("git diff --staged --name-only", rendered)
        self.assertIn("git diff --staged", rendered)

    def test_u04_guided_document_uses_shared_actions_as_a_public_route(self) -> None:
        """U04 renders its full real two-child workflow only when it is guided publicly."""
        ...
        self.assertIn(document_id, manifest_publication.guided_labs)
        build_preview_site(self.root, self.out, release_sha="d" * 40)
        self.assertTrue((self.out / "labs" / document_id / "index.html").is_file())

    def test_u04_public_render_places_the_real_lifecycle_before_check_and_safe_reset(self) -> None:
        """Catches a published U04 retaining the former hard stop or unavailable writer."""
        document_id = "U04-initialize-projects"
        document = preview_site._read_markdown_document(
            self.root,
            Path("academy/tracks/power-user/U04-initialize-projects.md"),
        ...
        self.assertIn("Prepare safely", rendered)
        ...
        writer_card = rendered.index('data-action-id="U04-write-binding-report"')
        check_card = rendered.index('data-action-id="U04-check-status"')
        reset_card = rendered.index('data-action-id="U04-reset-retry"')
        self.assertLess(prepare_card, first_future_card)
        self.assertLess(first_future_card, writer_card)
        self.assertLess(writer_card, check_card)
        self.assertLess(check_card, reset_card)
        self.assertNotIn("Future private-source walkthrough", rendered)
        self.assertNotIn("not runnable with Preview 0.17", rendered)
        self.assertIn('data-action-id="U04-write-binding-report"', rendered)
        self.assertIn('data-copy-target="command-U04-write-binding-report-windows"', rendered)
        self.assertIn("archive both child histories", rendered)
    ...
    def test_p08_public_guide_renders_all_action_cards_with_their_execution_identity(self) -> None:
        """Catches P08 action cards disappearing from its public route."""
        ...
        self.assertTrue(
            (self.out / "labs" / "P08-repository-hygiene" / "index.html").is_file()
        )
    ...
    def test_public_p04_action_contract_creates_a_public_lesson_page(self) -> None:
        ...
        self.assertIn('href="labs/P04-review-a-dependency/index.html"', index)
    ...
    def test_public_p03_renders_choice_copy_and_installed_preview_operations(self) -> None:
        """Catches rendered P03 cards losing learner choice or public installed commands."""
        manifest = load_action_manifest(self.root, "P03-record-an-adr")
        actions = {action.id: action for action in manifest.actions}

        self.assertIn("P03-request-decision-analysis", actions)
        choice = preview_site._render_action(actions["P03-request-decision-analysis"])
        for expected in (
        ...
        self.assertEqual(choice.count('class="command-copy"'), 9)
        ...
        for action_id in ("P03-prepare", "P03-check", "P03-reset"):
            with self.subTest(action=action_id):
                ...
                self.assertIn("preview-0.32", rendered)
    ...
    def test_f01_direct_entry_requires_home_setup_and_does_not_misstate_the_next_guided_lesson(self) -> None:
        ...
        self.assertIn(
            'href="../../index.html#complete-these-five-setup-steps-before-f01">'
            'five Academy Home setup steps</a> before Prepare.',
            html,
        )
    ...
    def test_home_exposes_the_five_f01_prerequisites_as_a_direct_linked_sequence(self) -> None:
        """Catches F01 sending a new learner to an unnumbered or indirect Home setup list."""
        build_preview_site(self.root, self.out, release_sha="e" * 40)
        home = (self.out / "index.html").read_text(encoding="utf-8")

        self.assertIn('id="complete-these-five-setup-steps-before-f01"', home)
        self.assertIn('<ol>', home)
        for target, label in (
            ("#create-your-practice-fork", "Create your practice fork"),
            ("#clone-it-to-your-computer", "Clone it to your computer"),
            ("#enter-the-cloned-repository", "Enter the cloned repository"),
            ("#verify-and-install-the-academy-tools", "Verify and install the Academy tools"),
            ("#run-readiness-checks", "Run readiness checks"),
        ):
            self.assertIn(f'href="{target}">{label}</a>', home)

    def test_foundation_next_steps_do_not_skip_required_f03(self) -> None:
        """Catches F02 routing a new learner around the required F03 lesson."""
        build_preview_site(self.root, self.out, release_sha="e" * 40)
        f02 = (self.out / "labs" / "F02-orient-to-state" / "index.html").read_text(
            encoding="utf-8"
        )
        f03 = (self.out / "labs" / "F03-work-the-board" / "index.html").read_text(
            encoding="utf-8"
        )

        self.assertIn('href="../F03-work-the-board/index.html"', f02)
        self.assertNotIn('Continue with <a href="../F04-fix-with-evidence/index.html"', f02)
        self.assertIn('href="../F02-orient-to-state/index.html"', f03)
        self.assertIn('href="../F04-fix-with-evidence/index.html"', f03)

    def test_p06_public_guided_document_renders_copyable_actions(self) -> None:
        """Catches the public P06 route losing its action-card execution identity."""
    ...
    def test_build_emits_the_complete_guided_practitioner_route_sequence(self) -> None:
        """Catches an accepted Practitioner lesson missing from the public release."""
        build_preview_site(self.root, self.out, release_sha="a" * 40)
        ...
        self.assertTrue((self.out / "labs" / "F03-work-the-board" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "F04-fix-with-evidence" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P01-feature-through-plan" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P02-commit-review-pr" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P03-record-an-adr" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P04-review-a-dependency" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P05-checkpoint-remediation" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P06-context-drift-recovery" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P07-threat-model" / "index.html").is_file())
        self.assertTrue((self.out / "labs" / "P08-repository-hygiene" / "index.html").is_file())
        index = (self.out / "index.html").read_text(encoding="utf-8")
        self.assertIn('href="labs/P05-checkpoint-remediation/index.html"', index)
        self.assertNotIn("<strong>Practitioner</strong>: P06 through P08. Guided rewrites are in progress.", index)
    ...
    def test_public_site_publishes_f04_guidance_and_keeps_practitioner_status_only(self) -> None:
        ...
        self.assertIn('data-action-id="F03-check"', f03)
    ...
    def test_static_checker_describes_the_exact_empty_coming_next_boundary(self) -> None:
        ...
        index.write_text(
            html.replace(
                marker,
                '<ul class="coming-next"><li>U99 is not published.</li></ul>\n' + marker,
            ),
            encoding="utf-8",
        )
    ...
    r"(?:guided )?Preview 0\.32 (?:inventory|guided-rewrite sequence)$",
        ):
    ...
    def test_published_guided_cli_commands_resolve_through_the_academy_parser(self) -> None:
        ...
        expected = {
            ...
            ("P01-feature-through-plan", "P01-prepare"): ("prepare", "P01-feature-through-plan"),
            ("P01-feature-through-plan", "P01-check"): ("check", "P01-feature-through-plan"),
            ("P01-feature-through-plan", "P01-reset-retry"): ("reset", "P01-feature-through-plan"),
            ...
            ("P02-commit-review-pr", "P02-prepare"): ("prepare", "P02-commit-review-pr"),
            ("P02-commit-review-pr", "P02-record-receipt"): ("record", "P02-commit-review-pr"),
            ("P02-commit-review-pr", "P02-check"): ("check", "P02-commit-review-pr"),
            ("P02-commit-review-pr", "P02-reset"): ("reset", "P02-commit-review-pr"),
            ("P03-record-an-adr", "P03-prepare"): ("prepare", "P03-record-an-adr"),
            ("P03-record-an-adr", "P03-check"): ("check", "P03-record-an-adr"),
            ("P03-record-an-adr", "P03-reset"): ("reset", "P03-record-an-adr"),
            ("P04-review-a-dependency", "P04-prepare"): ("prepare", "P04-review-a-dependency"),
            ("P04-review-a-dependency", "P04-check"): ("check", "P04-review-a-dependency"),
            ("P04-review-a-dependency", "P04-reset-retry"): ("reset", "P04-review-a-dependency"),
            ("P05-checkpoint-remediation", "P05-prepare"): ("prepare", "P05-checkpoint-remediation"),
            ("P05-checkpoint-remediation", "P05-check"): ("check", "P05-checkpoint-remediation"),
            ("P05-checkpoint-remediation", "P05-reset-retry"): ("reset", "P05-checkpoint-remediation"),
            ("P06-context-drift-recovery", "P06-prepare"): ("prepare", "P06-context-drift-recovery"),
            ("P06-context-drift-recovery", "P06-write-handoff"): ("write-handoff", "P06-context-drift-recovery"),
            ("P06-context-drift-recovery", "P06-check"): ("check", "P06-context-drift-recovery"),
            ("P06-context-drift-recovery", "P06-reset-retry"): ("reset", "P06-context-drift-recovery"),
            ("P07-threat-model", "P07-prepare"): ("prepare", "P07-threat-model"),
            ("P07-threat-model", "P07-check"): ("check", "P07-threat-model"),
            ("P07-threat-model", "P07-reset"): ("reset", "P07-threat-model"),
            ("P08-repository-hygiene", "P08-prepare"): ("prepare", "P08-repository-hygiene"),
            ("P08-repository-hygiene", "P08-check"): ("check", "P08-repository-hygiene"),
            ("P08-repository-hygiene", "P08-reset-retry"): ("reset", "P08-repository-hygiene"),
            ("U01-autonomous-sprint", "U01-prepare-attempt"): ("prepare", "U01-autonomous-sprint"),
            ("U01-autonomous-sprint", "U01-check-status"): ("check", "U01-autonomous-sprint"),
            ("U01-autonomous-sprint", "U01-reset-retry"): ("reset", "U01-autonomous-sprint"),
            ("U02-override-audit-metrics", "U02-prepare"): ("prepare", "U02-override-audit-metrics"),
            ("U02-override-audit-metrics", "U02-check"): ("check", "U02-override-audit-metrics"),
            ("U02-override-audit-metrics", "U02-reset"): ("reset", "U02-override-audit-metrics"),
            ("U03-refactor-chore-release", "U03-prepare"): ("prepare", "U03-refactor-chore-release"),
            ("U03-refactor-chore-release", "U03-check"): ("check", "U03-refactor-chore-release"),
            ("U03-refactor-chore-release", "U03-reset"): ("reset", "U03-refactor-chore-release"),
            ("U04-initialize-projects", "U04-prepare-attempt"): ("prepare", "U04-initialize-projects"),
            ("U04-initialize-projects", "U04-write-binding-report"): ("write-report", "U04-initialize-projects"),
            ("U04-initialize-projects", "U04-check-status"): ("check", "U04-initialize-projects"),
            ("U04-initialize-projects", "U04-reset-retry"): ("reset", "U04-initialize-projects"),
            ("U05-debug-spike-conflict", "U05-prepare-attempt"): ("prepare", "U05-debug-spike-conflict"),
            ("U05-debug-spike-conflict", "U05-check-status"): ("check", "U05-debug-spike-conflict"),
            ("U05-debug-spike-conflict", "U05-reset-retry"): ("reset", "U05-debug-spike-conflict"),
            ("U06-preview-and-advanced-surfaces", "U06-prepare-attempt"): ("prepare", "U06-preview-and-advanced-surfaces"),
            ("U06-preview-and-advanced-surfaces", "U06-check-status"): ("check", "U06-preview-and-advanced-surfaces"),
            ("U06-preview-and-advanced-surfaces", "U06-reset-retry"): ("reset", "U06-preview-and-advanced-surfaces"),
            ("U07-capstone", "U07-prepare"): ("prepare", "U07-capstone"),
            ("U07-capstone", "U07-check"): ("check", "U07-capstone"),
            ("U07-capstone", "U07-reset-retry"): ("reset", "U07-capstone"),
        }
                ...
                for variant in action.variants:
                    ...
                    for line in variant.command.splitlines():
                        ...
                        normalized = (
                            ...
                            .replace("(Resolve-Path -LiteralPath '<learner-repository>').Path", ".")
                            .replace("$learnerRepository", ".")
                            .replace('"$(cd \'<learner-repository>\' && pwd)"', ".")
                            .replace('"$PWD"', ".")
    ...
    def test_home_states_preview_scope_prerequisites_pacing_and_exact_workflow(self) -> None:
        ...
        self.assertIn("U01-U07 are published in this release", html)
        ...
        self.assertIn("<strong>Current release inventory.</strong>", html)
        self.assertIn("<strong>Course boundary.</strong>", html)
        self.assertNotIn('class="coming-next"', html)
    ...
    def test_home_teaches_every_prerequisite_before_first_use(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                "https://github.com/arbiterForge/arbiter-academy/blob/preview-0.32/install/install.ps1",
                "https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.32/install.ps1.sha256",
                "https://github.com/arbiterForge/arbiter-academy/blob/preview-0.32/install/install.sh",
                "https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.32/install.sh.sha256",
            ),
        )
        ...
        self.assertIn("installer digest did not match Preview 0.32", install_commands["windows"])
        self.assertLess(
        ...
        for operating_system in ("macos", "linux"):
            ...
            self.assertIn("installer digest did not match Preview 0.32", command)
        ...
        self.assertIn(
            'href="https://github.com/arbiterForge/arbiter-academy/blob/preview-0.32/install/install.ps1"',
            html,
        )
        self.assertIn(
            'href="https://github.com/arbiterForge/arbiter-academy/blob/preview-0.32/install/install.sh"',
            html,
        )
        self.assertIn(
        ...
        for state in (
            "<strong>Current release inventory.</strong>",
    ...
    def test_build_rejects_missing_or_out_of_boundary_discussion_url_before_writing(self) -> None:
        ...
        source = self._copy_public_source()
        manifest_path = source / "academy" / "publication" / "preview-0.32.json"
    ...
    def test_build_emits_the_exact_reviewed_file_inventory_and_index_boundary(self) -> None:
        ...
        self.assertIn('<meta name="academy-release" content="preview-0.32">', index)
    ...
    def test_static_checker_rejects_public_inventory_and_asset_mutations(self) -> None:
        """Catches stale release identity or a changed public inventory in Preview 0.32 output."""
        ...
        cases = (
            ...
            (
                "runnable P08 link removed",
                "replace",
                Path("index.html"),
                'href="labs/P08-repository-hygiene/index.html"',
                'href="labs/P05-checkpoint-remediation/index.html"',
            ),
            (
    ...
    def test_complete_preview_guidance_describes_its_graduation_boundary(self) -> None:
        """Catches a complete public inventory retaining the withheld-course boundary."""
        manifest = load_preview_manifest(self.root)
        self.assertEqual(len(manifest.available_labs), len(LAB_INVENTORY))
    ...
    def test_readme_matches_the_complete_preview_zero_twenty_seven_public_lesson_boundary(self) -> None:
        """Catches the repository entrypoint drifting from the rendered public Academy routes."""
        readme = (self.root / "README.md").read_text(encoding="utf-8")

        self.assertIn("Preview 0.32 publishes nineteen guided labs:", readme)
        self.assertIn(
            "https://arbiterforge.github.io/arbiter-academy/labs/F03-work-the-board/",
            readme,
        )
        for lab_id in (
            "F03-work-the-board",
    ...
    def test_static_checker_rejects_html_release_identity_drift(self) -> None:
        ...
        index = self.out / "index.html"
        html = index.read_text(encoding="utf-8")
        current = '<meta name="academy-release" content="preview-0.32">'
    ...
    def test_build_renders_complete_reviewed_lesson_bodies(self) -> None:
        ...
        for lab_id in (
            ...
            with self.subTest(lab=lab_id):
                rendered = (self.out / "labs" / lab_id / "index.html").read_text(encoding="utf-8")
                self.assertIn('<article class="academy-content">', rendered)
                self.assertIn('<p class="lesson-publication-status">Guided lesson</p>', rendered)
                self.assertIn(f'data-action-id="{lab_id.partition("-")[0]}-prepare"', rendered)
        self.assertTrue((self.out / "labs" / "P04-review-a-dependency" / "index.html").exists())
        self.assertTrue((self.out / "labs" / "P05-checkpoint-remediation" / "index.html").exists())
    ...
    def test_u06_private_document_uses_shared_actions_without_a_public_route(self) -> None:
        """U06 renders as a complete public lesson in Preview 0.32."""
        ...
        for inventory in (public.available_labs, public.runnable_labs, public.guided_labs):
            self.assertIn(lab_id, inventory)
        build_preview_site(self.root, self.out, release_sha="a" * 40)
        self.assertTrue((self.out / "labs" / lab_id / "index.html").is_file())
    ...
    def _copy_public_source(self, label: str = "source") -> Path:
        ...
        shutil.copy2(
            self.root / "academy" / "publication" / "preview-0.32.json",
            academy / "publication" / "preview-0.32.json",
        )
...
class PublicU02PreviewContractTests(unittest.TestCase):
    def test_u02_guide_uses_shared_action_renderer_for_the_real_h05_boundary(self) -> None:
        """Catches public U02 claiming an override or omitting its observation limitation."""
        root = Path(__file__).parents[1]
        document = preview_site._read_markdown_document(
            root,
        ...
        self.assertNotIn("{{action:", content)
        self.assertNotIn("```", guide)
        self.assertIn("does not practice `$ca-override`", guide)
        self.assertIn("cannot prove the event chronology", guide)
        self.assertIn("U02 is a published Power User lesson in Preview 0.32", guide)
        ...
        self.assertIn("only committed learner artifact", guide)
        self.assertIn("H-05 refusal", guide)
        self.assertIn("one child commit", guide)
...
class U03PreviewContractTests(unittest.TestCase):
    def test_u03_guide_uses_shared_action_renderer_with_local_only_boundary(self) -> None:
        """Catches U03 losing its route contract or claiming a remote release."""
        root = Path(__file__).parents[1]
        document = preview_site._read_markdown_document(
            root,
        ...
        self.assertNotIn("{{action:", content)
        self.assertNotIn("```", guide)
        self.assertNotIn("future private", guide.casefold())
        self.assertIn("local annotated", guide)
        self.assertIn("does not prove a remote tag", guide)
