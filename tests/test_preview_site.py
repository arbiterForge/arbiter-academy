...
class PreviewSiteTests(unittest.TestCase):
    def test_preview_zero_nineteen_publishes_u06_and_the_guided_inventory(self) -> None:
        ...
        manifest_path = publication / "preview-0.19.json"
        ...
        self.assertIn("Preview 0.19", index)
    ...
    def test_shared_release_chrome_derives_from_the_validated_preview_manifest(self) -> None:
        ...
        current = source / "academy" / "publication" / "preview-0.19.json"
    ...
    def test_public_p03_renders_choice_copy_and_installed_preview_operations(self) -> None:
        ...
        for action_id in ("P03-prepare", "P03-check", "P03-reset"):
            with self.subTest(action=action_id):
                ...
                self.assertIn("preview-0.19", rendered)
    ...
    def test_static_checker_describes_the_exact_empty_coming_next_boundary(self) -> None:
        ...
        r"(?:guided )?Preview 0\.19 (?:inventory|guided-rewrite sequence)$",
        ):
    ...
    def test_home_states_preview_scope_prerequisites_pacing_and_exact_workflow(self) -> None:
        ...
        self.assertIn("U07. Guided rewrite is in progress.", html)
        ...
        self.assertIn('class="coming-next"', html)
    ...
    def test_home_teaches_every_prerequisite_before_first_use(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                "https://github.com/arbiterForge/arbiter-academy/blob/preview-0.19/install/install.ps1",
                "https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.19/install.ps1.sha256",
                "https://github.com/arbiterForge/arbiter-academy/blob/preview-0.19/install/install.sh",
                "https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.19/install.sh.sha256",
            ),
        )
        ...
        self.assertIn(
            'href="https://github.com/arbiterForge/arbiter-academy/blob/preview-0.19/install/install.ps1"',
            html,
        )
        self.assertIn(
            'href="https://github.com/arbiterForge/arbiter-academy/blob/preview-0.19/install/install.sh"',
            html,
        )
        self.assertIn(
    ...
    def test_build_rejects_missing_or_out_of_boundary_discussion_url_before_writing(self) -> None:
        ...
        source = self._copy_public_source()
        manifest_path = source / "academy" / "publication" / "preview-0.19.json"
    ...
    def test_build_emits_the_exact_reviewed_file_inventory_and_index_boundary(self) -> None:
        ...
        self.assertIn('<meta name="academy-release" content="preview-0.19">', index)
        self.assertIn("Current release inventory.", index)
        self.assertIn("Graduation is unavailable until the complete 19-lab course is published.", index)
        self.assertNotIn("Preview 0.11", index)
        self.assertIn("coming-next", index)
        self.assertIn("U07.", index)
        self.assertTrue((self.out / "labs" / "P08-repository-hygiene" / "index.html").is_file())
        self.assertIn("P08-repository-hygiene", index)
    ...
    def test_preview_guidance_does_not_offer_graduation_before_the_course_is_published(self) -> None:
        ...
        for surface, text in (("README", readme), ("home", rendered_home)):
            with self.subTest(surface=surface):
                ...
                self.assertIn("Graduation is unavailable until", normalized)
                self.assertIn("complete 19-lab course", normalized)
                self.assertNotRegex(
                    text,
    ...
    def test_readme_matches_the_preview_zero_seventeen_public_lesson_boundary(self) -> None:
        ...
        self.assertIn("Preview 0.19 publishes seventeen guided labs:", readme)
        for lab_id in (
            "P04-review-a-dependency",
            "P05-checkpoint-remediation",
            "P06-context-drift-recovery",
    ...
    def test_static_checker_rejects_html_release_identity_drift(self) -> None:
        ...
        index = self.out / "index.html"
        html = index.read_text(encoding="utf-8")
        current = '<meta name="academy-release" content="preview-0.19">'
    ...
    def _copy_public_source(self, label: str = "source") -> Path:
        ...
        shutil.copy2(
            self.root / "academy" / "publication" / "preview-0.19.json",
            academy / "publication" / "preview-0.19.json",
        )
