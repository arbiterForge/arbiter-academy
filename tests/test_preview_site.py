...
class PreviewSiteTests(unittest.TestCase):
    def test_preview_zero_twenty_four_withholds_f03_from_the_guided_inventory(self) -> None:
        """Catches an invalid F03 path leaking into the public Preview artifact."""
        ...
        manifest_path = publication / "preview-0.25.json"
        ...
        self.assertFalse((self.out / "labs" / "F03-work-the-board" / "index.html").exists())
        ...
        self.assertIn("Preview 0.25", index)
    ...
    def test_shared_release_chrome_derives_from_the_validated_preview_manifest(self) -> None:
        ...
        current = source / "academy" / "publication" / "preview-0.25.json"
    ...
    def test_public_p03_renders_choice_copy_and_installed_preview_operations(self) -> None:
        ...
        for action_id in ("P03-prepare", "P03-check", "P03-reset"):
            with self.subTest(action=action_id):
                ...
                self.assertIn("preview-0.25", rendered)
    ...
    def test_public_site_publishes_f04_guidance_and_keeps_practitioner_status_only(self) -> None:
        ...
        self.assertFalse((self.out / "labs" / "F03-work-the-board" / "index.html").exists())
        self.assertNotIn('href="labs/F03-work-the-board/index.html"', index)
    ...
    def test_static_checker_describes_the_exact_empty_coming_next_boundary(self) -> None:
        ...
        r"(?:guided )?Preview 0\.25 (?:inventory|guided-rewrite sequence)$",
        ):
    ...
    def test_home_teaches_every_prerequisite_before_first_use(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                "https://github.com/arbiterForge/arbiter-academy/blob/preview-0.25/install/install.ps1",
                "https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.25/install.ps1.sha256",
                "https://github.com/arbiterForge/arbiter-academy/blob/preview-0.25/install/install.sh",
                "https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.25/install.sh.sha256",
            ),
        )
        ...
        self.assertIn(
            'href="https://github.com/arbiterForge/arbiter-academy/blob/preview-0.25/install/install.ps1"',
            html,
        )
        self.assertIn(
            'href="https://github.com/arbiterForge/arbiter-academy/blob/preview-0.25/install/install.sh"',
            html,
        )
        self.assertIn(
    ...
    def test_build_rejects_missing_or_out_of_boundary_discussion_url_before_writing(self) -> None:
        ...
        source = self._copy_public_source()
        manifest_path = source / "academy" / "publication" / "preview-0.25.json"
    ...
    def test_build_emits_the_exact_reviewed_file_inventory_and_index_boundary(self) -> None:
        ...
        self.assertIn('<meta name="academy-release" content="preview-0.25">', index)
        self.assertIn("Current release inventory.", index)
        self.assertIn("Graduation is unavailable until the withheld F03 lesson has a complete accepted guided path.", index)
    ...
    def test_static_checker_rejects_public_inventory_and_asset_mutations(self) -> None:
        """Catches stale release identity or a changed public inventory in Preview 0.25 output."""
    ...
    def test_withheld_preview_guidance_describes_its_graduation_boundary(self) -> None:
        """Catches a withheld lesson being represented as a completed course."""
        manifest = load_preview_manifest(self.root)
        self.assertEqual(len(manifest.available_labs), len(LAB_INVENTORY) - 1)
        ...
        for surface, text in (("README", readme), ("home", rendered_home)):
            with self.subTest(surface=surface):
                ...
                self.assertIn("Graduation is unavailable until the withheld F03 lesson has a complete accepted guided path.", normalized)
    ...
    def test_readme_matches_the_complete_preview_zero_twenty_one_public_lesson_boundary(self) -> None:
        ...
        self.assertIn("Preview 0.25 publishes eighteen guided labs:", readme)
        self.assertNotIn(
    ...
    def test_static_checker_rejects_html_release_identity_drift(self) -> None:
        ...
        index = self.out / "index.html"
        html = index.read_text(encoding="utf-8")
        current = '<meta name="academy-release" content="preview-0.25">'
    ...
    def test_u06_private_document_uses_shared_actions_without_a_public_route(self) -> None:
        """U06 renders as a complete public lesson in Preview 0.25."""
    ...
    def _copy_public_source(self, label: str = "source") -> Path:
        ...
        shutil.copy2(
            self.root / "academy" / "publication" / "preview-0.25.json",
            academy / "publication" / "preview-0.25.json",
        )
...
class PublicU02PreviewContractTests(unittest.TestCase):
    def test_u02_guide_uses_shared_action_renderer_for_the_real_h05_boundary(self) -> None:
        ...
        self.assertIn("U02 is a published Power User lesson in Preview 0.25", guide)
