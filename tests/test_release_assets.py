...
class ReleaseAssetBuilderTests(unittest.TestCase):
    def test_preview_zero_thirteen_is_the_only_current_candidate_identity(self) -> None:
        ...
        self.assertIn("academy/publication/preview-0.13.json", package)
    ...
    def test_preview_zero_nine_retains_its_reviewed_immutable_history(self) -> None:
        """Preview 0.13 promotion must not rewrite Preview 0.9 tag semantics."""
        self.assertEqual(
    ...
    def test_current_preview_uses_its_tag_as_soon_as_publication_creates_it(self) -> None:
        """A published Preview 0.13 must stop selecting mutable candidate bytes."""
        expected = "a" * 40
        ...
        self.assertEqual(
            ...
            ["git", "rev-parse", "--verify", "refs/tags/preview-0.13^{commit}"],
        )
...
class InstallerBehaviorTests(unittest.TestCase):
    ...
    def test_powershell_redirect_validator_rejects_nondefault_https_ports(self) -> None:
        ...
        harness.write_text(
            ...
            "$github = [Uri]'https://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.13/file.zip'\n"
