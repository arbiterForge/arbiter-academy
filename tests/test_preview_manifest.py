...
PREVIEW_0_17 = [*PREVIEW_0_16, "U04-initialize-projects"]
COMING_NEXT_0_16 = ["U04-initialize-projects", "U05-debug-spike-conflict", "U06-preview-and-advanced-surfaces", "U07-capstone"]
COMING_NEXT = ["U05-debug-spike-conflict", "U06-preview-and-advanced-surfaces", "U07-capstone"]
...
class PreviewManifestTests(unittest.TestCase):
    ...
    def test_preview_zero_sixteen_promotes_only_the_accepted_u03_closure(self) -> None:
        ...
        self.assertEqual(manifest.available_labs, tuple(PREVIEW_0_17))
        self.assertEqual(manifest.runnable_labs, tuple(PREVIEW_0_17))
        self.assertEqual(manifest.guided_labs, tuple(PREVIEW_0_17))
        self.assertEqual(manifest.coming_next, tuple(COMING_NEXT))

    def test_preview_zero_seventeen_promotes_only_the_accepted_u04_closure(self) -> None:
        """U04 becomes public only with its three accepted Power User prerequisites."""
        manifest = load_preview_manifest(self.root)
        ...
        self.assertEqual(
            ...
            ("U05-debug-spike-conflict", "U06-preview-and-advanced-surfaces", "U07-capstone"),
        )
    ...
    def make_manifest(self, root: Path | None = None, **changes: object) -> dict[str, object]:
        ...
        manifest: dict[str, object] = {
            ...
            "available_labs": PREVIEW_0_17,
            "runnable_labs": PREVIEW_0_17,
            "guided_labs": PREVIEW_0_17,
    ...
    def test_preview_zero_fifteen_keeps_u02_through_u07_private(self) -> None:
        ...
        self.assertEqual(manifest.release, "preview-0.17")
        expected_public = tuple(PREVIEW_0_17)
    ...
    def test_preview_zero_twelve_is_current_and_preserves_preview_zero_eleven_history(self) -> None:
        ...
        self.assertTrue((publication / "preview-0.17.json").is_file())
        expected = tuple(PREVIEW_0_17)
        self.assertEqual(manifest.release, "preview-0.17")
    ...
    def test_preview_manifest_rejects_the_immediately_stale_release_identity(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "release must be preview-0.17"):
    ...
    def test_preview_manifest_rejects_mismatched_compatibility_lists(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "available_labs must equal runnable_labs"):
            validate_preview_manifest(
                self.root,
                self.make_manifest(available_labs=PREVIEW_0_17[:-1]),
            )
    ...
    def test_publication_schema_pins_sixteen_guided_public_labs(self) -> None:
        ...
        self.assertEqual(schema["properties"]["release"]["const"], "preview-0.17")
        self.assertEqual(schema["properties"]["lesson_contract_version"]["const"], 1)
        self.assertEqual((available["minItems"], available["maxItems"]), (16, 16))
        self.assertEqual(
            [entry["const"] for entry in available["prefixItems"]],
            PREVIEW_0_17,
        )
        self.assertEqual((runnable["minItems"], runnable["maxItems"]), (16, 16))
        self.assertEqual(
    ...
    def test_load_preview_manifest_returns_the_reviewed_public_boundary(self) -> None:
        ...
        self.assertEqual(
            release_files,
            ["preview-0.10.json", "preview-0.11.json", "preview-0.12.json", "preview-0.13.json", "preview-0.14.json", "preview-0.15.json", "preview-0.16.json", "preview-0.17.json", "preview-0.4.json", "preview-0.9.json"],
        )
