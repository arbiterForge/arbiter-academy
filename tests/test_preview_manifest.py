...
class PreviewManifestTests(unittest.TestCase):
    ...
    def test_preview_manifest_rejects_the_immediately_stale_release_identity(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "release must be preview-0.22"):
    ...
    def test_repository_checkout_keeps_identity_bound_inputs_lf(self) -> None:
        ...
        self.assertNotIn(b"\r\n", catalog)
        manifest = json.loads(
            (self.root / "academy" / "publication" / "preview-0.22.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(
    ...
    def test_publication_schema_pins_sixteen_guided_public_labs(self) -> None:
        ...
        self.assertEqual(schema["properties"]["release"]["const"], "preview-0.22")
    ...
    def test_load_preview_manifest_returns_the_reviewed_public_boundary(self) -> None:
        ...
        self.assertEqual(
            release_files,
            ["preview-0.10.json", "preview-0.11.json", "preview-0.12.json", "preview-0.13.json", "preview-0.14.json", "preview-0.15.json", "preview-0.16.json", "preview-0.17.json", "preview-0.18.json", "preview-0.19.json", "preview-0.20.json", "preview-0.21.json", "preview-0.22.json", "preview-0.4.json", "preview-0.9.json"],
        )
