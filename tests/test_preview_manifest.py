...
class PreviewManifestTests(unittest.TestCase):
    ...
    def make_manifest(self, root: Path | None = None, **changes: object) -> dict[str, object]:
        ...
        manifest: dict[str, object] = {
            ...
            "available_labs": PREVIEW_0_11,
            "runnable_labs": PREVIEW_0_11,
            "guided_labs": PREVIEW_0_11,
    ...
    def test_preview_zero_eleven_promotes_only_p04_and_p05_after_the_public_prerequisite_closure(self) -> None:
        """Catches P04/P05 being absent, P06-P08 routing, or a stale release identity."""
        manifest = load_preview_manifest(self.root)

        self.assertEqual(manifest.release, "preview-0.11")
        self.assertEqual(manifest.available_labs, tuple(PREVIEW_0_11))
        self.assertEqual(manifest.runnable_labs, tuple(PREVIEW_0_11))
        self.assertEqual(manifest.guided_labs, tuple(PREVIEW_0_11))
        self.assertEqual(
            manifest.coming_next,
            ("P06-context-drift-recovery", "P07-threat-model", "P08-repository-hygiene"),
        )

    def test_preview_zero_eleven_publishes_guided_p04_p05_and_keeps_later_practitioner_labs_private(self) -> None:
        """P04 and P05 join the complete public prerequisite path only after review."""
        manifest = load_preview_manifest(self.root)

        self.assertEqual(manifest.release, "preview-0.11")
        expected_public = tuple(PREVIEW_0_11)
    ...
    def test_preview_zero_eleven_is_current_and_preserves_preview_zero_ten_history(self) -> None:
    ...
    def test_checked_in_public_boundary_is_a_fresh_preview_zero_eleven_for_f01_through_p05(self) -> None:
    ...
    def test_preview_manifest_rejects_the_immediately_stale_release_identity(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "release must be preview-0.11"):
    ...
    def test_preview_manifest_rejects_mismatched_compatibility_lists(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "available_labs must equal runnable_labs"):
            validate_preview_manifest(
                self.root,
                self.make_manifest(available_labs=PREVIEW_0_11[:-1]),
            )
    ...
    def test_publication_schema_pins_nine_guided_public_labs_and_status_only_follow_ups(self) -> None:
        """Catches the schema omitting P04/P05 or publishing an unaccepted later Practitioner rewrite."""
        schema = json.loads(
            (self.root / "academy" / "publication" / "preview-manifest.schema.json").read_text(
                encoding="utf-8"
            )
        ...
        self.assertEqual(schema["properties"]["release"]["const"], "preview-0.11")
        ...
        self.assertEqual((runnable["minItems"], runnable["maxItems"]), (9, 9))
        self.assertEqual(
        ...
        self.assertEqual((coming_next["minItems"], coming_next["maxItems"]), (3, 3))
        self.assertEqual(
    ...
    def test_load_preview_manifest_returns_the_reviewed_public_boundary(self) -> None:
        ...
        self.assertEqual(
            release_files,
            ["preview-0.10.json", "preview-0.11.json", "preview-0.4.json", "preview-0.9.json"],
        )
