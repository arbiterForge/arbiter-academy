    ...
    def test_installed_academy_verifier_targets_a_separate_learner_repository(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertTrue(
                any(
                    name.endswith(
                        "share/arbiter-academy/academy/publication/preview-0.28.json"
                    )
                    for name in names
                ),
                "preview-0.28.json",
            ...
            self.assertTrue(
                any(name.endswith("/academy/actions/U04-initialize-projects.json") for name in names),
                "the public wheel must distribute the accepted U04 action contract",
            )
            self.assertTrue(
