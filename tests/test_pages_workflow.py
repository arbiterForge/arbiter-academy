...
RELEASE_ASSETS = (
    ...
    "install.sh",
    "install.sh.sha256",
    "arbiter-academy-preview-0.9.zip",
    "arbiter-academy-preview-0.9.zip.sha256",
)
...
class PagesWorkflowContractTests(unittest.TestCase):
    ...
    def test_pages_release_gate_targets_the_exact_preview_zero_eight_candidate(self) -> None:
        """Catches Pages reproducing or accepting superseded Preview 0.7 release assets."""
        self.assertEqual(
            RELEASE_ASSETS,
            (
                "install.ps1",
        ...
        self.assertRegex(release_job, r"(?m)^      RELEASE_TAG: preview-0\.9\s*$")
        self.assertRegex(release_job, r"(?m)^      RELEASE_EPOCH: 1786665600\s*$")
    ...
    def test_release_metadata_validator_rejects_noncanonical_checksums_and_inventory_drift(self) -> None:
        ...
        def run_validator(
            ...
            with tempfile.TemporaryDirectory() as directory:
                ...
                checksums = {
                    ...
                    "arbiter-academy-preview-0.9.zip.sha256": (
                        f"{'c' * 64}  arbiter-academy-preview-0.9.zip\n".encode()
                    ),
                }
                ...
                environment = os.environ.copy()
                environment["RELEASE_TAG"] = "preview-0.9"
            ...
            with self.subTest(release_state=mutation):
                ...
                with tempfile.TemporaryDirectory() as directory:
                    ...
                    for name, digest in (
                        ...
                        ("install.sh.sha256", "b"),
                        ("arbiter-academy-preview-0.9.zip.sha256", "c"),
                    ):
        ...
        for mutation in (
            {"state": "new"},
            {"state": None},
            {"browser_download_url": "http://github.com/arbiterForge/arbiter-academy/releases/download/preview-0.9/install.ps1"},
            {"browser_download_url": "https://evil.example/preview-0.9/install.ps1"},
        ):
