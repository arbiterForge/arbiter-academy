...
Preview 0.12 refuses U02 because the Power User track is unavailable and not published there. Prepare, Check, and Reset must leave the repository unchanged. Do not create an attempt, evidence files, or an override record to simulate a published lesson.
...
Do not run the following actions from Preview 0.12. They document the private course contract only after a future private surface has prepared an attempt.
...
Preview 0.12 still refuses the U02 checkpoint action and leaves the repository unchanged. In a future private prepared attempt, its local checkpoint would validate the deterministic evidence contract, but it would not prove a human or hosted fact.
