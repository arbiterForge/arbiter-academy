...
U04 is future private-source material and is not runnable with Preview 0.15. The current release
...
The optional card below proves only that installed Preview 0.15 refuses Prepare. It does not create
the two children or authorize the remaining lifecycle.

{{action:U04-prepare-attempt}}

After the refusal, stop and use a published Preview 0.15 lab.
...
lifecycle contract, but they are not executable with Preview 0.15.
...
Only after both future child commits are clean may the Academy parent bind them. Preview 0.15 cannot
...
Current Preview 0.15 Check is only an optional refusal probe. It cannot verify the future lifecycle.

{{action:U04-check-status}}

## Recover or continue

Preview 0.15 Reset is only an optional refusal probe and cannot recover this unpublished lesson.
...
through the same byte renderer. Until then, Preview 0.15 refuses the commands before those U04
mechanics run.
