...
This source lesson is written for the Academy website. The Power User track remains private, so it does not create a public U03 route or change Preview 0.14.

## Know before you begin

Read the boundary before running anything. Preview 0.14 refuses U03 prepare, check, and reset unchanged. That is correct: this release has no runnable U03 attempt.
...
Preview 0.14 cannot validate this private contract. Its refusal is the current public result.

{{action:U03-check}}

## Recover or continue

Do not destroy a failed future attempt to make it look clean. Preserve the state and use the private reset rules only when that future release supplies them. Preview 0.14 has no U03 attempt to reset.
