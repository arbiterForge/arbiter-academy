...
checkpoint_command: {{action:U06-check-status}}
next_lab: U07-capstone
---
...
U06 is a private source lesson. Academy Preview 0.14 is the installed public release, and it does
...
{{action:U06-prepare-attempt}}

{{action:U06-inspect-scenario}}
...
{{action:U06-create-contained-diff}}

{{action:U06-inspect-preview-input}}

{{action:U06-run-read-only-preview}}

The two names are easy to confuse. Academy Preview 0.14 names an installed release and its public
...
{{action:U06-assess-preview-output}}

{{action:U06-stage-candidate}}

{{action:U06-commit-candidate}}
...
{{action:U06-classify-advanced-surfaces}}
...
{{action:U06-write-binding-report}}

{{action:U06-inspect-binding-report}}

{{action:U06-stage-report}}

{{action:U06-commit-report}}
...
{{action:U06-confirm-clean}}

{{action:U06-check-status}}
...
installed Academy Preview 0.14 Check still refuses U06 until an accepted release carries this source
contract.

## Recover or continue

{{action:U06-reset-retry}}
...
The candidate is a reviewed, frozen teaching target. Equivalent prose is not evidence when the Check
requires exact final bytes.
